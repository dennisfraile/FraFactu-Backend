        ...
        public async Task<ActionResult<LoginResponseDto>> Login([FromBody] LoginDto loginDto)
        {
            ...
            switch (result.Status)
            {
                ...
                case LoginStatus.Bloqueado:
                    Response.Headers["Retry-After"] = result.SegundosRestantes.ToString();
                    var minutos = (int)Math.Ceiling(result.SegundosRestantes / 60.0);
                    return StatusCode(StatusCodes.Status423Locked, new
                    {
                        message = $"Demasiados intentos fallidos. Intenta de nuevo en ~{minutos} minuto(s)."
                    });
                default:
