...
public class CompraExternaService : ICompraExternaService
{
    ...
    public async Task<CompraExternaDto> ConfirmarAsync(int id, int emisorId, ConfirmarCompraDto? dto = null)
    {
        ...
        try
        {
            ...
            // F3: si el Hub tiene SmartInventory activo, encolamos un evento
            // al outbox para replicar los movimientos de esta compra. Best-effort:
            // si falla el encolado, log pero NO rompemos la confirmacion porque
            // la compra ya esta CONFIRMADA en Smartix y el stock interno actualizado.
            await EncolarReplicaInventarioAsync(compra);
    ...
    /// <summary>
    /// F3 (Plan inventario desde DTE): si el Emisor tiene la app SmartInventory
    /// activa, persiste un job en el outbox <c>IntegracionInventarioPendiente</c>
    /// con todos los detalles de la compra. El consumer lo enviara en background.
    /// Idempotencia: <c>MovimientoIdExterno = "compra-{compraId}"</c> garantiza
    /// que un reintento desde SmartInventory devuelva 409 si ya fue procesado.
    /// </summary>
    private async Task EncolarReplicaInventarioAsync(CompraExterna compra)
    {
        try
        {
            // Sucursal.Emisor ya viene incluido por el ConfirmarAsync.
            var emisor = compra.Sucursal?.Emisor;
            if (emisor is null || !emisor.TieneSmartInventoryActiva || emisor.HubId is null)
                return;

            var itemsInventario = compra.Detalles
                .Where(d => d.EsParaInventario && d.Producto != null && d.Bodega != null)
                .ToList();
            if (itemsInventario.Count == 0) return;

            var movimientoIdExterno = $"compra-{compra.Id}";

            // Si ya hay un job para esta compra (caso de re-confirmar tras error
            // transitorio), no duplicamos: el consumer ya lo va a procesar.
            var yaEncolado = await _context.IntegracionInventarioPendientes
                .AnyAsync(j => j.MovimientoIdExterno == movimientoIdExterno);
            if (yaEncolado) return;

            var payload = new Application.DTOs.Integraciones.SmartInventoryMovimientoRequestDto
            {
                OrganizacionId = emisor.HubId.Value,
                MovimientoIdExterno = movimientoIdExterno,
                TipoDocumento = "COMPRA_EXTERNA",
                DocumentoOrigenId = compra.Id,
                NumeroDocumento = compra.NumeroFactura,
                FechaMovimiento = compra.FechaConfirmacion ?? GetUtcNow(),
                Observaciones = $"Compra externa confirmada - Proveedor: {compra.ProveedorId}",
                Items = itemsInventario.Select(d => new Application.DTOs.Integraciones.SmartInventoryMovimientoItemDto
                {
                    // Identidad estable cross-app: el id del producto en Smartix.
                    // SmartInventory matchea por esto (no por codigo) para no sumar
                    // el stock a un producto ajeno con codigo coincidente.
                    ProductoIdExterno = d.Producto!.Id,
                    CodigoProducto = d.Producto!.Codigo,
                    NombreProducto = d.Producto.Nombre,
                    BodegaNombre = d.Bodega!.Nombre,
                    // Id de la sucursal en SmartHub (1:1 con la sucursal de la compra).
                    // SmartInventory lo usa para que la bodega replicada NO quede sin
                    // sucursal (si no, el filtro Sub-plan 8 la oculta). El wizard scopea
                    // las bodegas a la sucursal de la compra, asi que es la correcta.
                    SucursalIdExterno = compra.Sucursal?.HubSucursalId,
                    Cantidad = d.Cantidad,
                    CostoUnitario = d.CostoUnitario,
                    TipoInventario = d.Producto.TipoInventario.ToString(),
                    // Solo enviamos campos de activo fijo cuando aplica.
                    FechaAdquisicion = d.Producto.TipoInventario == Domain.Enums.TipoInventario.MobiliarioEquipo
                        ? d.Producto.FechaAdquisicion : null,
                    AniosVidaUtil = d.Producto.TipoInventario == Domain.Enums.TipoInventario.MobiliarioEquipo
                        ? d.Producto.AniosVidaUtil : null,
                    ValorActual = d.Producto.TipoInventario == Domain.Enums.TipoInventario.MobiliarioEquipo
                        ? d.Producto.ValorActual : null,
                    ValorResidual = d.Producto.TipoInventario == Domain.Enums.TipoInventario.MobiliarioEquipo
                        ? d.Producto.ValorResidual : null
                }).ToList()
            };

            _context.IntegracionInventarioPendientes.Add(new Domain.Entities.IntegracionInventarioPendiente
            {
                EmisorId = emisor.Id,
                TipoEvento = "MOVIMIENTO_ENTRADA",
                PayloadJson = System.Text.Json.JsonSerializer.Serialize(payload),
                Estado = Domain.Enums.EstadoIntegracionInventario.ENCOLADO,
                MovimientoIdExterno = movimientoIdExterno,
                FechaCreacion = GetUtcNow()
            });
            await _context.SaveChangesAsync();
        }
        catch (Exception ex)
        {
            // Best-effort: la compra ya esta confirmada y el stock interno
            // actualizado. Si fallamos al encolar el evento, el admin tendra
            // que reintentar manualmente desde la pantalla de integraciones
            // (a implementar en F4) o por SQL directo.
            _logger?.LogError(ex,
                "[CompraExternaService] No se pudo encolar replica de inventario para compra {CompraId}",
                compra.Id);
        }
    }

    /// <summary>
    /// Tras editar una compra confirmada con cambio de items, encola una corrección
    /// a SmartInventory por DELTA NETO por (producto, bodega). El consumer hace
    /// stock += cantidad, así que un delta negativo decrementa correctamente.
    /// Best-effort: si falla, el inventario interno de Smartix ya quedó corregido.
    /// Id versionado "compra-{id}-edit{n}" para sortear la idempotencia del espejo.
    /// </summary>
    private async Task EncolarCorreccionInventarioAsync(
        CompraExterna compra,
        List<(int ProductoId, int BodegaId, decimal Cantidad)> itemsViejos,
        List<CrearCompraDetalleDto> itemsNuevos)
    {
        try
        {
            var emisor = compra.Sucursal?.Emisor;
            ...
            // Delta neto por (producto, bodega): nuevo - viejo, solo items de inventario.
            var deltas = new Dictionary<(int prod, int bod), decimal>();
            foreach (var v in itemsViejos)
            {
                var k = (v.ProductoId, v.BodegaId);
                deltas[k] = deltas.GetValueOrDefault(k) - v.Cantidad;
            }
            foreach (var n in itemsNuevos.Where(d => d.EsParaInventario))
            {
                var k = (n.ProductoId, n.BodegaId);
                deltas[k] = deltas.GetValueOrDefault(k) + n.Cantidad;
            }

            var deltasNoCero = deltas.Where(kv => kv.Value != 0m).ToList();
            if (deltasNoCero.Count == 0) return;

            // Costo a reportar por item: el del nuevo detalle si existe, si no el promedio guardado.
            var costoPorProducto = itemsNuevos
                .GroupBy(d => (d.ProductoId, d.BodegaId))
                .ToDictionary(g => g.Key, g => g.First().CostoUnitario);

            var items = new List<Application.DTOs.Integraciones.SmartInventoryMovimientoItemDto>();
            foreach (var kv in deltasNoCero)
            {
                var producto = await _context.ProductosServicios.FindAsync(kv.Key.prod);
                var bodega = await _context.Bodegas.FindAsync(kv.Key.bod);
                if (producto is null || bodega is null) continue;

                var costo = costoPorProducto.GetValueOrDefault(kv.Key,
                    (await _context.StocksBodega
                        .FirstOrDefaultAsync(s => s.ProductoId == kv.Key.prod && s.BodegaId == kv.Key.bod))?.CostoPromedio ?? 0m);

                items.Add(new Application.DTOs.Integraciones.SmartInventoryMovimientoItemDto
                {
                    ProductoIdExterno = producto.Id,
                    CodigoProducto = producto.Codigo,
                    NombreProducto = producto.Nombre,
                    BodegaNombre = bodega.Nombre,
                    SucursalIdExterno = compra.Sucursal?.HubSucursalId,
                    Cantidad = kv.Value, // delta neto (puede ser negativo)
                    CostoUnitario = costo,
                    TipoInventario = producto.TipoInventario.ToString(),
                    FechaAdquisicion = producto.TipoInventario == Domain.Enums.TipoInventario.MobiliarioEquipo ? producto.FechaAdquisicion : null,
                    AniosVidaUtil = producto.TipoInventario == Domain.Enums.TipoInventario.MobiliarioEquipo ? producto.AniosVidaUtil : null,
                    ValorActual = producto.TipoInventario == Domain.Enums.TipoInventario.MobiliarioEquipo ? producto.ValorActual : null,
                    ValorResidual = producto.TipoInventario == Domain.Enums.TipoInventario.MobiliarioEquipo ? producto.ValorResidual : null
                });
            }
            if (items.Count == 0) return;

            // Id versionado: cuenta de jobs previos de ESTA compra (replica exacta "compra-{id}"
            // o ediciones "compra-{id}-..."). Se usa el guion final para no matchear
            // otras compras con prefijo numerico compartido (ej. compra-12 vs compra-120).
            var idReplica = $"compra-{compra.Id}";
            var prefijoEdicion = $"compra-{compra.Id}-";
            var versionesPrevias = await _context.IntegracionInventarioPendientes
                .CountAsync(j => j.MovimientoIdExterno != null &&
                                 (j.MovimientoIdExterno == idReplica ||
                                  j.MovimientoIdExterno.StartsWith(prefijoEdicion)));
            var movimientoIdExterno = $"compra-{compra.Id}-edit{versionesPrevias}";
            ...
            var payload = new Application.DTOs.Integraciones.SmartInventoryMovimientoRequestDto
            {
                ...
                NumeroDocumento = compra.NumeroFactura,
                FechaMovimiento = GetUtcNow(),
                Observaciones = $"Corrección por edición de compra - Proveedor: {compra.ProveedorId}",
                Items = items
            };
        ...
        {
            _logger?.LogError(ex,
                "[CompraExternaService] No se pudo encolar corrección de inventario para compra {CompraId}",
                compra.Id);
        }
    }
    ...
    public async Task<CompraExternaDto> EditarConfirmadaAsync(int id, ActualizarCompraExternaDto dto, int emisorId)
    {
        ...
        try
        {
            ...
            // 6. Corrección best-effort a SmartInventory (por delta neto).
            await EncolarCorreccionInventarioAsync(compra, itemsViejos, dto.Detalles);
